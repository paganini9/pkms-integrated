...
| v1.6 | 2026-07-12 | **CD-15** — `Design.env`·`Project.target_env` 의 **닫힌 enum(`Winter`) 해제**. 온톨로지는 거버넌스로 자라는데(Ozone·Crack…) 설계 표현이 닫혀 있으면 저작한 지식이 판정에 **영원히 도달할 수 없다** | 04·05·07 |
...
### CD-15. `Design.env` 는 닫힌 enum 이 아니다 — 온톨로지가 자라면 설계도 그것을 표현할 수 있어야 한다 — v1.6

**포스트-g3 실 검증에서 값을 치르고 배웠다.** 저작 루프(T-93)를 놓고 보니, 엔지니어가 "오존 노출이 기준을
초과하면 고무 블레이드에 균열이 생긴다"를 저작하고 거버넌스로 `Ozone`·`Crack` 을 온톨로지에 편입해도,
**설계가 오존 환경을 표현할 수 없어서** 그 지식은 어떤 판정에도 닿지 못했다. `env` 가 `"Winter"` 하나로
닫혀 있었기 때문이다. 지식은 자라는데 그 지식을 물어볼 **질문(설계)** 이 자라지 못하면 루프는 닫히지 않는다.

확정: `Design.env`·`Project.target_env` 는 **온톨로지가 아는 `EnvCondition` 의 로컬네임**(개방 문자열)이다.
- 온톨로지가 모르는 값은 **어떤 규칙과도 매칭되지 않는다** — 조용히 무시될 뿐 판정을 왜곡하지 않는다(무해).
- 값 검증은 enum 이 아니라 **온톨로지 소속**이 한다(T-73·T-94 와 같은 원칙: 진실원은 온톨로지다).
- 시드 동작 불변 — `env:"Winter"` 는 그대로 `dom:Winter` 로 해석된다.
