...
| T-71 | 08 | LLM-as-Judge 루브릭(정확성·근거성·안전성·RAG충분성) | T-70, T-88 | **done** | `run_judge.py`. **발견: solar-pro3 심판이 비결정적**(같은 답변 grounded 2/3→1/3) → **하드 게이트=결정론 루브릭**(정확성 599·근거성 sources≥1·미검증근거 0·안전성 fail-closed) 0 위반(2R). Solar 심판은 best-of-3 자문(비게이트). |
| T-72 | 08 | 실패 케이스(타임아웃·range위반·수치누락·도메인밖·롤백) | T-70 | **done** | `run_failure_cases.py` — §4 HTTP 관측분 Solar 실 스택 4/4(range 저장차단 409·수치누락 판정보류·도메인밖 근거없음·저장 멱등). 심층(Breaker·롤백)은 유닛 참조. |
...
| T-85 | 04 | 상위 온톨로지 승인 후 **TTL 영속화** | T-55(done) | **done** | 승인 add 변경을 영속 오버레이 TTL(data/upper_overlay.ttl, 시드 분리)에 저장. `_onto`·`classes` 가 시드+오버레이 로드 → **재기동 후 유지**(새 인스턴스 테스트로 증명). 미승인 409·미지원 op 422. |
...
> 근거 `docs/저작-KR이슈-분석.md`. 순서: **T-90(측정)** → T-89(복합어·어휘) → T-91(인과 reification).
