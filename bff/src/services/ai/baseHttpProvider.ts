...
// T-73 이후: 도메인 경계 판정은 **결정론적 온톨로지 소속(지식서비스 unknown_concept)**이 한다.
// 그래서 추출기는 도메인을 스스로 거르지 않고 **등장 개념을 모두** 뽑는다(도메인 밖 개념도).
// 도메인 밖 개념은 접지 게이트가 unknown 으로 잡아 fail-closed 한다(가드레일이 프롬프트 규율에
// 의존하지 않게 됨). 이렇게 해야 in-domain 리콜(공집합 추출 문제)도 함께 해결된다.
export const SYS_EXTRACT =
  "너는 지식 추출기다. 입력 문장에 **등장하는 명사 개념을 빠짐없이** 추출한다. " +
  "와이퍼 블레이드 도메인 개념(재질 고무·실리콘, 차종 중형 SUV·소형 세단, 부품 와이퍼 암·스프링, " +
  "증상 소음·끝단 떨림, 환경 겨울철 저온, 속성 경도·길이·압력·형상)은 물론, " +
  "**다른 제품·부위(타이어·엔진·자전거 체인·노트북·귀마개·신발 밑창 등)가 등장해도 그대로 추출**한다. " +
  "도메인 소속 여부는 네가 판정하지 않는다(다음 단계가 결정론으로 판정). type·predicate 는 주어진 " +
  "열거형 중 가장 가까운 것으로. 평서문이든 질문이든 등장 개념을 추출한다. 판정하지 말고 추출만 한다.";
const SYS_PARSE_REQ =
