...
// T-91 인과 프레임: 조건(환경)은 **증상에** 건다(증상 conditionedOn 조건). 부품에 조건을 걸면 도메인 위반이라
// 지식서비스가 (기전·조건·증상)을 한 Causation 노드로 재화(reify)할 수 있게 트리플을 낸다.
export const SYS_EXTRACT =
  ...
  "**인과 프레임(중요)**: 원인(부품·재질·속성)은 causes/aggravates 로 **증상**에 걸고, 환경 조건은 반드시 " +
  "**증상**에 건다 — `증상 conditionedOn 조건`. 조건을 부품·재질의 주어로 걸지 마라(예 '블레이드 conditionedOn 겨울' 금지). " +
  "그러면 (기전·조건·증상)이 한 인과로 묶인다. " +
  "type·predicate 는 주어진 열거형 중 가장 가까운 것. 평서문이든 질문이든 추출한다. 판정하지 말고 추출만 한다. " +
  "**아래 예시는 형식 설명일 뿐이다 — 예시의 개념(겨울철·블레이드·소음 등)을 그대로 베끼지 말고, 실제 입력 문장에 있는 것만 뽑아라.**\n" +
  "예시1(복합어 분해) '고무 블레이드' → " +
